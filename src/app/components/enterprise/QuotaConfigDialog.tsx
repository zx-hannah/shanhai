...
interface QuotaConfigDialogProps {
  ...
  onClose: () => void;
  onSave: (config: { type: QuotaType; amount?: number; period?: Period }) => void;
}

const PERIOD_OPTIONS: { key: Period; label: string; resetLabel: string }[] = [
  { key: "monthly", label: "每月", resetLabel: "每月1日恢复" },
  { key: "quarterly", label: "每季度", resetLabel: "每季度首日恢复" },
  { key: "yearly", label: "每年", resetLabel: "每年1月1日恢复" },
];

export function QuotaConfigDialog({ title, subtitle, currentConfig, onClose, onSave }: QuotaConfigDialogProps) {
  const [quotaType, setQuotaType] = useState<QuotaType>(currentConfig?.type || "periodic");
  const [amount, setAmount] = useState(currentConfig?.amount?.toString() || "20000");
  const [period, setPeriod] = useState<Period>(currentConfig?.period || "monthly");
