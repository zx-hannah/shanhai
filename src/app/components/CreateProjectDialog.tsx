...
interface EditProps {
  project: { name: string; tokenTotal: number; cover?: string };
  onClose: () => void;
  onSave: (project: { name: string; tokenTotal: number; cover?: string }) => void;
}

export function EditProjectDialog({ project, onClose, onSave }: EditProps) {
  const [name, setName] = useState(project.name);
  const [tokenTotal, setTokenTotal] = useState(String(project.tokenTotal));
  const [coverPreview, setCoverPreview] = useState<string | null>(project.cover ?? null);
  const fileInputRef = useRef<HTMLInputElement>(null);
  ...
  const handleSave = () => {
    ...
    const quota = parseInt(tokenTotal, 10);
    if (!tokenTotal.trim() || isNaN(quota) || quota <= 0) { toast.error("请输入有效的配额数量"); return; }
    onSave({ name: name.trim(), tokenTotal: quota, cover: coverPreview ?? undefined });
    onClose();
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4"
          style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
          <h3 className="text-white">编辑项目</h3>
          <button onClick={onClose} style={{ color: "rgba(255,255,255,0.4)" }}>
            <X size={18} />
          </button>
        </div>
